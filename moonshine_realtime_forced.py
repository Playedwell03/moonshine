...
def _best_forced_class(text):
    key = _normalize(text)
    best_label = None
    best_score = -1.0

    for label, aliases in FORCED_CLASSES.items():
...
class ConsoleListener(TranscriptEventListener):
    ...
    def _map_text(self, text):
        return _best_forced_class(text)
    ...
    def on_line_completed(self, event):
        if getattr(event, "line", None):
            raw = (event.line.text or "").strip()
            mapped = self._map_text(raw)
            if self.final_only:
                if self.drop_question and raw.endswith("?"):
                    return
                if self.min_chars and len(raw) < self.min_chars:
                    return
...
def main():
    parser = argparse.ArgumentParser(description="Realtime mic transcription with forced 4-class command mapping.")
    ...
    parser.add_argument(
        "--final-only",
        action="store_true",
        help="Only print completed lines (recommended).",
    )
    parser.add_argument(
        "--merge-window",
        type=float,
